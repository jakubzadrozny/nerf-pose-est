...
def extra_args(parser):
    ...
    parser.add_argument(
        "--eval_batch_size", "-E", type=int, default=100000, help="Eval batch size"
    )
    parser.add_argument(
        "--freeze_enc",
        action="store_true",
...
class PixelNeRFTrainer(trainlib.Trainer):
    def __init__(self, net, dset_train, dset_test, args, conf, z, device):
        super().__init__(net, dset_train, dset_test,
                         args, conf["train"], device=device)
        ...
        print(
            "lambda coarse {} and fine {}".format(
                self.lambda_coarse, self.lambda_fine)
        )
    ...
    def calc_losses(self, data, is_train=True, global_step=0):
        ...
        all_focals = data["focal"].to(device=device)  # (SB)
        all_c = data["c"].to(device=device)  # (SB)
        ...
        src_focals = util.batched_index_select_nd(
            all_focals, image_ord
        )  # (SB, NS, 2)
        src_c = util.batched_index_select_nd(
            all_c, image_ord
        ) if all_c is not None else None
        # (SB, NS, 2)
    ...
    def eval_step(self, data, global_step):
        ...
        losses = self.calc_losses(
            data, is_train=False, global_step=global_step)
        renderer.train()
        return losses
    ...
    def vis_step(self, data, global_step, idx=None):
        ...
        focal = data["focal"][batch_idx].to(device=device)  # (1)
        ...
        gt = images_0to1[view_dest].permute(
            1, 2, 0).cpu().numpy().reshape(H, W, 3)
        with torch.no_grad():
            ...
            net.encode(
                ...
                focal[views_src].unsqueeze(0),
                c=c[views_src].unsqueeze(0) if c is not None else None,
            )
        ...
        print("c rgb min {} max {}".format(
            rgb_coarse_np.min(), rgb_coarse_np.max()))
        print(
        ...
        if using_fine:
            print("f rgb min {} max {}".format(
                rgb_fine_np.min(), rgb_fine_np.max()))
            print(
